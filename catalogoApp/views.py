...
def cargarCliente(request):
    if 'userName' not in request.session:
        ...
        for cliente in lista_clientes:
            ...
            # Verificamos si es restaurante o no

            if cliente.restaurante == True:
                restaurante_string = "Si"
            else:
                restaurante_string = "No"   

            formateado = [cliente.id,cliente.nombre,cliente.direccion,cliente.comuna,cliente.telefono,cliente.cantidadPedidos,debe,cliente.extra, restaurante_string]
        ...
        if request.method == 'GET':
            ...
            # Entra por POST, envia datos para crear cliente
            ...
            if len(lista_similares) == 0:
                ...
                try:
                    cliente = Clientes.objects.create(nombre=nombre,direccion=direccion,comuna=comuna,extra=extra,telefono=telefono,cantidadPedidos=0,deuda=False, restaurante=restaurante)
                    cliente.save()
    ...
    if 'userName' not in request.session:
        ...
        if request.method == 'GET' and len(request.GET['id']) != 0:
            ...
            try:
                ...
                if usuarioModificar.restaurante == False:
                    restaurante = "No es"
                else:
                    restaurante = "Si es"

                datosModificar = [usuarioModificar.id,usuarioModificar.nombre,usuarioModificar.direccion,usuarioModificar.comuna,usuarioModificar.telefono,usuarioModificar.cantidadPedidos,debe,usuarioModificar.extra,restaurante]
            ...
            restauranteModificado = request.POST['restauranteModificado']
            ...
            if len(restauranteModificado) != 0:
                if restauranteModificado == "s":
                    restauranteModificado = True
                else:
                    restauranteModificado = False

                usuarioModificado.update(restaurante=restauranteModificado)
